...
int main() {
    ...
    while (window.isOpen()) {
        ...
        for (auto& p : planets) {
            if (&p == &sun) continue;  // Le soleil n'est pas affecté par la gravité, pas de mise à jour
            ...
            // Détection des collisions (n'affecte pas le soleil)
            if (&p != &sun) {
